...
// TODO: Use @react-native-async-storage/async-storage instead of localStorage
...
export function loadDailyFortuneProfile() {
  try {
    const raw = storageGetItem(DAILY_FORTUNE_PROFILE_KEY);
    if (!raw) return null;
...
export function saveDailyFortuneProfile(profile) {
  try {
    const normalizedProfile = normalizeDailyFortuneProfile(profile);
    storageSetItem(DAILY_FORTUNE_PROFILE_KEY, JSON.stringify({
      ...normalizedProfile,
      savedAt: Date.now()
    }));
  } catch (e) {
...
export function clearDailyFortuneProfile() {
  try {
    storageRemoveItem(DAILY_FORTUNE_PROFILE_KEY);
  } catch (e) {
